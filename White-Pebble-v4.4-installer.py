...
def download(forge_version):
    ...
    try:
        ...
        response = requests.get('https://github.com/Ztrolix/ProjectWhitePebble/releases/download/versions/White-Pebble-v4.4.zip')
        with open("White-Pebble-v4.4.zip", 'wb') as f:
...
def extract():
    ...
    try:
    	with ZipFile("White-Pebble-v4.4.zip", 'r') as zObject:

    		# Extracting all the members of the zip
    		# into a specific location.
    		zObject.extractall(
    			path="C:\\White-Pebble\\instances\\White-Pebble v4.4")
    	f = open("4.4.txt", "w")
    	f.write("True")
    	f.close()
    except:
